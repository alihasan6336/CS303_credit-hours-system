...
export const createStudentAccount = async (req: Request, res: Response): Promise<void> => {
    try {
        const { fullName, universityId, email, password, major, level, currentSemester, completedCreditHours } = req.body;
...
export const getStudentAcademicRecord = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const enrollments = await Enrollment.find({ student: new mongoose.Types.ObjectId(id as string) })
        ...
        res.status(200).json({
            ...
            totalCreditsCompleted: student.completedCreditHours || 0,
